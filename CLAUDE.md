...
## 코인 현물 전략 규칙 (V23, 재정의 2026-04-30)

V23 = 모든 자산 1D 단일 + drift trigger. 4h 멤버 제거, cron 1일 1회로 단순화.
...
- Engine: `trade/coin_live_engine.py` (SCHEMA_VERSION='V23', drift_fire 평가)
...
## 선물 전략 규칙 (V23, 재정의 2026-04-30)

### 구성

- D_SMA42 단일 (ENSEMBLE_WEIGHTS = {'D_SMA42': 1.0})
- snap_interval_bars=95, n_snapshots=5 (95 = 5 × 19, prime stagger=19) — 2026-05-04 갱신 (3→5, 57→95)
- canary_hyst=0.015, universe_size=3, cap=1/3
- drift_threshold=0.03 (2026-05-04 갱신, 0.05 → 0.03 반응성 ↑)
- 단독 sleeve (backtest_futures_full.py, 펀딩 fix 후, 2020-10~2026-05, tx 0.0006, L3): CAGR +256%, MDD -63.1%, Cal 4.05
- 참고: 이전 표기 Cal 13.41 은 futures_ensemble_engine.py 다른 엔진 결과. 정식 truth 는 backtest_futures_full.py 이고 펀딩 매칭 버그 fix (2026-05-14) 후 Cal 4.05

### 실행 파라미터

- 레버리지: 고정 3배 (floor=mid=ceiling=3)
- 스탑: 없음 (STOP_PCT=0)
- 캐시 가드: 없음 (STOP_GATE_CASH_THRESHOLD=0)
- 가드 없음 (2026-04-21~), 앙상블 분산이 유일 방어
- 거래비용: 0.04% (바이낸스 maker)
- 유지증거금: 0.4%
- 4h fetch 제거, 1d 1회 cron (`5 9 * * *`)

### V22 → V23 마이그레이션 (2026-04-30)
...
- 마이그레이션 스크립트: `trade/migrate_v22_to_v23.py`
- 운영 매뉴얼: `V23_OPERATION_MANUAL.md`

### 자산배분 (V23 갱신 2026-05-22, M 안 채택)

- 주식 60% / 업비트 25% / 바이낸스 15% (이전 70/15/15 → 신규 60/25/15)
- 코인 총 40% (spot 25 / fut 15 = 5:3 비율, fut 레버리지 비중 ↓ 로 변동성 drag 완화)
- 363 후보 unified rank-sum (n=72 windows) 결과: 6025 + T1(13)|T3U_can(20) 이 broad plateau center (T1=13 row × T3U_can 20-30 모두 Cal 3.80+, MDD -17.8 일관)
- 단일 5.4yr BT: Cal 3.97, CAGR 70.7%, MDD -17.8%, Sharpe 2.09, 5.4yr 20회 리밸
- 리밸 트리거 (OR 조합):
  - T1: half_turnover (sum|cur_w - tgt_w|/2) ≥ 13pp (M 안 unified rank champ, T3U_can 변동에 둔감)
...
- 트리거 발동 시 텔레그램 알림 + gmoh HTML 표시. 자금 이동은 수동.
- 자산간 강제 이동(카나리 레짐 전환) 없음 — 각 자산 내부 방어
- 리밸 시 전 자산 목표 비중으로 복원
- 채택 근거: marginal stability 분석 — T1(13) std 0.194 (T3U_can 변동 둔감), T3U_can(20) std 0.216 (T1 변동 둔감), 양쪽 avgRank 최우수. Cal min 이웃 worst 3.37 (안전 floor).
- Stock V23 drift trigger (2026-05-23 추가): drift_threshold=0.10. cur_w (cash 포함) vs combined target 의 half_turnover ≥ 10pp 면 즉시 rebal. 외부 cash inflow (자산간 alloc rebal) 즉시 deploy + 평상시 internal drift 도 catch. BT Cal +2.8% (1.09 → 1.12, MDD -14.85 동일). 코드: stock_engine_snap.py run_snapshot_ensemble + trade/executor_stock.py 동시 반영.
- alloc_transit phantom buffer (옵션 D, 2026-05-23 채택): 트리거 ON 시 trade_state.json 에 alloc_transit flag set. cap_ratio[sleeve] = min(1.0, target_sleeve_krw / current_sleeve_krw) 매일 갱신. 각 executor 가 effective_pv = actual_pv × cap_ratio 적용 (3자산 대칭). clear: ht ≤ 0.05. hysteresis gap 8pp (13pp → 5pp). 파일: recommend_personal.py (flag set/clear/update) + trade/{executor_stock,executor_coin,auto_trade_binance}.py (cap_ratio 적용).
...
- [ ] 백테스트 코드 업데이트 (`backtest_spot_barfreq.py` V22 / `legacy/backtest_official.py` legacy 참조)
...
- `V23_OPERATION_MANUAL.md`는 운영 기준 문서이고, 실제 truth는 코드와 상태 전이까지 포함한다.
