## [2026-08-27] 결정: 선물 시장메타 fail-closed 배포 — 문턱 6건 보강 + 위험축소 분리는 후속
tags: 선물, 안전망, 상장폐지, ai-debate, 배포
- 결정: 위 fail-open 제거를 실서버에 배포했다(19:06 KST, md5 2586b7f7538c14626f6f6493f48cb1cf, 백업 auto_trade_binance.py.bak_20260827_190612, 커밋 fa5c279). 메타를 못 얻으면 그 실행은 주문 전에 ABORT 한다. 메타 장애 시 위험축소(reduceOnly)까지 막히는 것은 알고 배포했고, close-only 분리는 실제 사건이 생기면 넣는다(사용자 판단).
- 근거: 라이브 로그 실측(binance_trade.log 2026-03-31~08-27, 321회) 시총 목록·거래소 심볼정보 조회 실패 0건. 차단이 성립하려면 이틀 연속 실패가 필요한데 다섯 달간 한 번도 없었다. 반면 fail-open 은 매일 상시 노출이었다. 이 전략은 스탑주문이 없고 위험축소 주기가 원래 24시간(09:05 크론 1회)이라 차단 비용은 크론 1회 실패와 같은 크기다.
- ai-debate 4회(run-20260827T091013Z / 093109Z / 094524Z / 095253Z)로 결함 8건을 고쳤다: 조기반환 검증 → 전수검사, 문턱 없음 → 거래소 200종·시총 고유 30·교집합 바닥, CG 미검증 → 형태·고유수·중복, ABORT 를 알림보다 먼저 기록, 저하 사유 집계(단일 불리언은 두번째 원인을 숨겼다), 행 개수 → 고유 심볼 집합, 급감 기준을 TTL 안쪽 캐시로 한정(만료 캐시가 poison pill 이 돼 무기한 ABORT 를 만들던 것).
- 실측이 잡은 자기 오류: 교집합 정상 범위를 35~40 으로 잘못 알고 바닥을 20 에 뒀는데 라이브 실측은 22~25 였다(321회 전부). 그대로 배포했으면 정상 변동에서 오ABORT 가 났다. 로그를 안 봤으면 못 잡았을 값이다 — 문턱은 추정이 아니라 관측으로 정한다.
- 미채택: 만료 뒤 첫 응답이 절대 문턱을 넘으면 잘려 있어도 승인되는 잔여 경로에 복수확인·운영자승인 재기준화를 붙이는 안(4차 arbiter 요구). 이틀 연속 실패 뒤 그 첫 응답이 하필 200~500종 사이로 잘려야 성립하는 경우라 예외처리만 늘어난다고 봤다. 같은 판정이 위험축소 분리를 다시 차단사유로 든 것도 3차에서 자기가 후속과제로 분리한 것을 새 근거 없이 되돌린 것이라 미채택.
- 워밍업 검증(cron 계정 ubuntu, 주문 계열 호출을 가드로 차단): universe 24종, 캐시 2개 생성(fetched_at 최신, uid 1001), 거래가능 심볼 524, cg 40행, binance_state.json 무변경, abort 로그 무변경.
- 되돌릴 조건: 설계대로 난 ABORT 는 롤백 사유가 아니다. 롤백은 ABORT 뒤 주문이 나가거나, 잘린 응답이 캐시를 덮거나, 정상 응답이 반복 거부되거나, 미처리 예외로 죽거나, 워밍업 밖에서 예상 밖 주문·포지션·레버리지 변경이 보일 때.
- 검증: tests/test_fut_market_meta_cache.py(부분응답 7종·만료 래치 3종 포함) + tests/test_fut_meta_abort_no_orders.py(main(--trade) 를 태워 주문 API 0회·ABORT 기록·텔레그램 예외 격리) 신설. 선물 스위트 8종 전부 PASS.

## [2026-08-27] 결정: 선물 거래소 심볼정보 fail-open 제거 — 고정 목록 폐기 + 36시간 캐시
tags: 선물, 안전망, 상장폐지, ai-debate
- 결정: refresh_universe 의 HARDCODED_UNIVERSE_FALLBACK(손으로 적은 20종) 경로를 삭제. 조회 성공 시 응답을 검증해 조회시각과 함께 원자 저장하고, 실패 시 36시간 이내 캐시만 쓰며 degraded 알림 1회. 캐시 부재·손상·나이불명·만료면 주문 생성 전 ABORT. CoinGecko 시총 목록에도 같은 TTL 적용(기존 캐시는 나이 제한이 없어 몇 달 전 순위로도 돌 수 있었다). 커밋 1986fce, 로컬만 — 서버 배포는 승인 대기.
- 근거: V25 매매경로의 다른 이상(시세조회 실패·마진모드 불일치·L 하향 사전매도 실패)은 전부 즉시 ABORT 인데 이 자리만 fail-open 이었다. 고정 목록은 갱신되지 않아 상장폐지 심볼이 남아도 안 걸러지고, 하필 조회가 실패한 그날 가장 낡아 있다. ai-debate run-20260827T084242Z 가 "확인된 유일한 결함" 으로 지목했다.
- 선행 게이트(중재자 지정, 코드 확인 완료): 위험축소 주문의 reduceOnly 적용 여부와 단방향 모드 보장. 둘 다 통과 — 주문 자리는 execute_rebalance(:1373 전량청산 True, :1376 부분감량 True)와 L 하향 사전매도(reduceOnly=True 고정) 둘뿐이고, 매도 수량은 보유수량×비율(비율<=1)이라 반전 불가, 단방향 모드는 매 실행 ensure 후 실패 시 ABORT. 게이트가 통과했으므로 단일 최우선 변경이 이 fail-open 제거로 확정됐다.
- 기각: 단순 fail-closed(조회 실패 즉시 매매 스킵). 조회 장애 한 번에 하루 리밸런싱을 통째로 버려 가용성 손실이 크다. 고정 목록 유지도 기각(미검증 신규 주문 위험).
- 정정: "주문이 거절될 뿐 손실은 안 난다" 는 세션 판단은 조건부로 틀렸다. 심볼 1개 이상이 전체 break 를 일으켜 다른 코인의 감량·레버리지 하향까지 막으므로 손실을 키울 수 있다. 다음 단계(비대칭 격리)의 근거.
- 남은 순서: (2) 주문 직전 검사 — 거래가능·규격조회 성공. 단 신규·증량과 축소·청산을 갈라 거래정지 심볼도 줄이는 주문은 허용. (3) 비대칭 격리 — 이상 시 증량 전면 동결, 정상 심볼 감량은 계속, 목표비중 재배분 없음. (4) 거래불가 심볼 재시도 억제(영구 제외 목록은 안 만듦). (5) 그림자 정상 5회. 각 단계 독립 커밋.
- 되돌릴 조건: 36시간 TTL 안에서 심볼 상태가 바뀌는 잔여 위험, 캐시 만료로 리밸런싱을 거른 날이 실제로 생기면 TTL 재검토. exchangeInfo 실패율 분포는 아직 안 재봤다.
- 검증: tests/test_fut_market_meta_cache.py 32항목 PASS(정상조회·12h캐시·40h만료·캐시없음·손상·나이불명·빈응답·심볼누락·비거래상태·정상캐시보존·교집합공백·상장폐지재현). 기존 선물 테스트 6종 회귀 없음.

## [2026-08-27] 결정: 선물 체결 밴드는 대칭 5% 유지 — 위험축소 매도 분리 기각
tags: 선물, 체결밴드, 리밸런싱, BT
- 결정: 매도·매수 대칭 5% 유지. 라이브(auto_trade_binance DELTA_THRESHOLD)·SSoT 엔진 모두 변경 0. 실험 토글 FUT_SELL_BAND 는 측정 후 되돌려 diff 0 확인.
- 근거: 합성 60/25/15 에서 현행 Cal 4.58 / 매도1% 4.57 / 매도0% 4.56, MDD 는 셋 다 -12.6% 로 동일. 사전 문턱(Calmar 0.3)의 15분의 1이라 성과로는 완전 동률. 반면 선물 슬리브 단독은 현행이 Cal 7.42 / MDD -38.3% 로 우위(매도1% 6.86 / -40.9%, 매도0% 6.82 / -40.9%)고 비용 5배에서는 현행이 117 구간 중 92 승으로 압도. 체결은 1.28~1.49배 증가. 얻는 것 없이 회전과 비용만 는다.
- 뒤집힌 가설: 계기는 08-26 레버리지 하향 3건 뒤 남은 과다노출 -2.1~-3.0% 가 밴드 안이라 안 팔린 것, 그리고 delta=(목표-현재)/현재 라 부족(+5.0%)은 발화하고 초과(-4.76%)는 발화 안 하는 방향 비대칭이었다. 그런데 매도 밴드를 없애자 슬리브 낙폭이 -38.3%→-40.9% 로 악화했다. 밴드 5% 가 하락 중 목표 미세변동에 따른 팔았다 되사는 왕복을 흡수하는 완충이었다. 구조가 이상해 보이는 것과 그 구조가 손해를 낳는 것은 다르고, 여기선 후자가 성립 안 함.
- 되돌릴 조건: 밴드 안 잔여 편차가 라이브에서 실제로 누적되는 게 관측될 때(현재 관측 2회차 08-26·08-27 모두 다음 봉에 부호 반전, 누적 없음). 그때도 밴드 폭이 아니라 "레버리지 변경으로 목표 명목이 줄어드는 경우는 밴드를 안 탄다" 는 이벤트 조건을 본다 — 전량청산·신규진입·카나리 OFF 청산·레버리지/마진 변경은 이미 밴드를 안 탄다.
- 실험/실패: 선물 체결 밴드의 매도쪽만 0% 또는 1% 로 분리하는 안 — 합성 Calmar 차이 0.02·MDD 동일, 슬리브 낙폭 2.6%p 악화, 비용5배 완패. 재시도 시 시간낭비.
- 보고서: reports/2026-08-27-fut-sell-band.html · 원시로그 logs/2026-08-27-fut-sell-band.log · 재현 패치 strategies/cap_defend/research/bt_fut_sell_band.engine.patch

## [2026-08-25] 발견: 합성 인용치는 상한이 아니라 하한 + 송금 지연은 일주일이 경계, 꼬리 방어는 주식이 한다
tags: 자산배분, 합성, 송금, 꼬리위험, BT
- 발견(A): 지금까지 인용해 온 합성 계산(매일 60/25/15 복귀, Cal 4.58)은 부풀린 상한이 아니었다. 현실 가정인 "트리거 발화 후 5영업일 뒤 송금" 이 Cal 4.77 로 더 낫다. 매일 되돌리기는 잘 가는 슬리브에서 매일 돈을 빼는 셈이라 손해다(앵커 사건구동 기각 때와 같은 원인 = 승자 조기 절단). 인용치 정정 불필요.
- 발견(B): 송금 지연 비용은 일주일이 경계다. 즉시 Cal 5.36 / 3영업일 4.81 / 5영업일 4.77 / 10영업일 3.62. MDD 는 -12.5% → -13.8% → -14.4% → -20.0%. 운영 규칙 = 배분 트리거 알림 오면 5영업일 안에 송금, 10영업일 넘기면 낙폭 1.6배. 5.8년 송금 횟수 8~13회(연 2회꼴)라 손으로 감당 가능.
- 발견(C): 송금을 아예 안 하면 Cal 은 4.77 로 같지만 CAGR +168.6%·MDD -35.3%·최종 297.8배·최종 비중이탈 82.9pp — 선물이 포트폴리오를 지배해 이미 60/25/15 가 아니다. 즉 배분 트리거는 수익 장치가 아니라 낙폭을 정해둔 수준에 묶는 장치다. 비율은 사용자 결정 사항이라 제안하지 않는다.
- 발견(D) 꼬리동조: 합성 최악 5% 일(73일)에 주식-현물 상관 -0.546, 주식-선물 -0.622 — 그런 날 주식이 반대로 움직여 손실을 깎았다. "위기에 다 같이 무너진다" 는 주식-코인 축에서 관측 안 됨. 반면 현물-선물은 전구간 0.694·꼬리 0.595 로 안 떨어져 합쳐서 40% 가 사실상 하나의 위험 덩어리다. 세 슬리브 동시 하락일 9.1%, 합성 최악 20일 중 10일. 최악 20일의 손실은 거의 전부 선물에서 왔다(선물 일간 -6.4~-25.6%).
- 한계: 카나리는 근사(20거래일 평균수익>0), 송금 비용 미반영, 지연은 고정 영업일, 슬리브 시계열은 BT 결과라 슬리브 자체의 BT-라이브 갭은 미포함.
- 되돌릴 조건: 송금 비용·환전 스프레드를 넣거나 실제 송금 이력이 쌓이면 (B) 의 경계값 재확인. (D) 는 선물 비중을 바꿀 때 다시 본다.
- 코드 변경 0(라이브·SSoT 무변경). 보고서 reports/2026-08-25-alloc-transfer-lag.html · 원시로그 logs/2026-08-25-alloc-transfer-lag.log
