"""
Cap Defend V11 Recommendation Script
====================================
...
def run_coin_strategy_v11(coin_universe, all_prices, target_date, log, is_today=True):
    """
    V11 Coin: V10 + Health-Check Forced Rebalancing
    Returns: Portfolio, Status, MetaData (Signal Dist, Next Candidates, Health Status), Log
    """
    date_str = target_date.date()
    log.append(f"<h3>🪙 코인 포트폴리오 (V11) ({date_str})</h3>")
...
def save_html(log_global, final_port, s_port, c_port, s_stat, c_stat, turnover, log_today, log_yesterday, date_today, asset_prices_krw, s_meta, c_meta, coin_health_status):
    ...
    <head>
        ...
        <title>Cap Defend V11 Recommendation</title>
        <style>
...
if __name__ == "__main__":
    ...
    # Coin Run (Today & Yest)
    log_today = []
    c_port, c_stat, c_meta, log_today = run_coin_strategy_v11(c_univ, prices, today, log_today, True)
    
    # Yesterday for Turnover
    c_port_y, _, _, _ = run_coin_strategy_v11(c_univ, prices, yest, [], False)
