...
# [2026-07-03 보안] 강제거래 엔드포인트 제거: /api/trade/upbit (업비트 풀-에쿼티 강제매수)와
# 헬퍼 run_trade_async 를 삭제. 사유: TRADE_PIN 이 과거 git 히스토리에 노출됐고 포트 5000 이
# 외부에 열려 있어, PIN 만으로 임의 강제매수를 트리거할 수 있는 실질적 노출이었다. 강제거래는
# 필요 시 서버에서 직접 run_executor.sh/run_trade.sh 로만 수행한다(웹 노출 금지).
# 복원이 필요하면 git 이력 참조 후 인증 강화(길고 랜덤한 PIN + 포트 내부화 + rate-limit) 선행.
