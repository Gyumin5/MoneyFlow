...
# 라이브 실측(2026-03-31~08-27, 321회): 교집합은 22~25종이다. 시총 상위 40 중에
# 바이낸스 선물에 없는 것과 스테이블코인이 빠져서 그렇다. 20 으로 두면 정상 변동
# (22)에서 두 칸 차이라 멀쩡한 실행이 멈춘다. 12 는 관측 최소의 절반이라 정상
# 변동에는 안 걸리고, 응답이 반토막 난 경우는 잡는다.
