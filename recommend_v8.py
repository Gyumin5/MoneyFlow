...
def run_crypto_strategy_v8(coin_universe: list, all_prices: dict, target_date: pd.Timestamp, log_for_date: list, is_today_run: bool) -> (dict, str, list):
    ...
    # --- v8 로직 변경점: Full Allocation ---
    coin_alloc_ratio = 1.0 if healthy_coins else 0.0
    cash_ratio = 1.0 - coin_alloc_ratio
    log_for_date.append(f"<p>    - <b>건강한 코인 수: {len(healthy_coins)}개 -> Full Allocation 적용 (코인 투자 비중: {coin_alloc_ratio:.0%})</b></p>")

    if not healthy_coins:
        return {CASH_ASSET: 1.0}, "건강한 코인 없음", log_for_date
    ...
    if not sharpe_scores:
        ...
        return {CASH_ASSET: 1.0}, "랭킹 계산 불가", log_for_date
    ...
    return final_coin_portfolio, f"강세장 (Full Allocation)", log_for_date
...
def save_portfolio_to_html(global_log, final_portfolio, stock_portfolio, coin_portfolio_today, stock_status, coin_status_today, portfolio_yesterday_coin_only, turnover, log_yesterday, log_today, date_yesterday, asset_prices_krw):
    ...
    <head>
        ...
        <title>자동 포트폴리오 추천 및 내 포트폴리오 턴오버 분석 (v8)</title>
        <style>
...
if __name__ == "__main__":
    ...
    log_today_coin_calc = []
    coin_portfolio_today, coin_status_today, log_today_coin_calc = run_crypto_strategy_v8(current_coin_universe, all_prices, date_today, log_today_coin_calc, is_today_run=True)
    
    log_yesterday_coin_calc = []
    coin_portfolio_yesterday, coin_status_yesterday, log_yesterday_coin_calc = run_crypto_strategy_v8(current_coin_universe, all_prices, date_yesterday, log_yesterday_coin_calc, is_today_run=False)
    ...
    try:
        print("\n- Fetching standard exchange rate from API...")
        response = requests.get("https://api.frankfurter.app/latest?from=USD&to=KRW", timeout=10)
        response.raise_for_status()
        usdt_krw_rate = response.json()['rates']['KRW']
        print(f"\n- 적용 환율: {usdt_krw_rate:,.2f} KRW/USD (Source: Forex API)")
        global_log.append(f"<p>- 적용 환율: {usdt_krw_rate:,.2f} KRW/USD (Source: Forex API)</p>")
    except Exception as e_api:
        print(f"\n- [경고] 표준 환율 API 조회 실패: {e_api}. Upbit USDT 환율로 대체합니다.")
        global_log.append(f"<p class='error'>- [경고] 표준 환율 API 조회 실패: {e_api}. Upbit USDT 환율로 대체합니다.</p>")
        try:
            usdt_krw_rate = pyupbit.get_current_price("KRW-USDT")
            if usdt_krw_rate is None: raise ValueError("Failed to fetch KRW-USDT rate from Upbit")
            print(f"\n- 적용 환율: {usdt_krw_rate:,.2f} KRW/USD (Source: Upbit KRW-USDT)")
            global_log.append(f"<p class='error'>- <b>주의:</b> 적용된 환율은 암호화폐 프리미엄(김치 프리미엄)이 포함된 Upbit의 USDT-KRW 가격이며, 실제 은행 환율과 다를 수 있습니다.</p>")
        except Exception as e_upbit:
            usdt_krw_rate = 1380.0 # Final Fallback
            print(f"\n- [경고] 모든 환율 정보 조회 실패. 기본값 {usdt_krw_rate:,.2f} KRW/USD 적용.")
            global_log.append(f"<p class='error'>- [경고] 모든 환율 정보 조회 실패. 기본값 {usdt_krw_rate:,.2f} KRW/USD 적용.</p>")

    asset_prices_krw = {}
    all_assets_in_port = set(final_portfolio.keys()) | set(coin_portfolio_today.keys())
    print("\n- Fetching KRW prices for assets in portfolio...")
    for asset in all_assets_in_port:
        if asset == CASH_ASSET:
            continue

        is_coin = asset.endswith('-USD')

        try:
            if is_coin:
                symbol = asset.replace('-USD', '')
                krw_ticker = f"KRW-{symbol}"
                krw_price = pyupbit.get_current_price(krw_ticker)
                if krw_price is not None:
                    asset_prices_krw[asset] = krw_price
                    print(f"  - Fetched {{asset}} ({{krw_ticker}}): {{krw_price:,.0f}} KRW")
                else:
                    # Fallback to USD conversion if direct KRW price fails
                    price_usd = all_prices[asset].iloc[-1]
                    asset_prices_krw[asset] = price_usd * usdt_krw_rate
                    print(f"  - Fetched {{asset}} (Fallback to USD): {{asset_prices_krw[asset]:,.0f}} KRW")
            else: # It's a stock
                if asset in all_prices:
                    price_usd = all_prices[asset].iloc[-1]
                    asset_prices_krw[asset] = price_usd * usdt_krw_rate
                    print(f"  - Fetched {{asset}} (USD): ${{price_usd:,.2f}} -> {{asset_prices_krw[asset]:,.0f}} KRW")
        except Exception as e:
            print(f"  - Failed to fetch price for {asset}: {e}")

    print("\n" + "=" * 60)
    print("               🏆 최종 v8 포트폴리오 추천 🏆")
    print("=" * 60)
