#!/usr/bin/env python3
"""M3 Engine Final — Codex exact simulate implementation.

Unit convention:
- v21_daily["equity"]: V21 standalone equity, normalized (start=1.0)
- c_delta: incremental PnL of C sleeve in same unit
- port_eq = v21_eq + c_delta
- qty × price, tx costs in the same normalized currency unit as equity
"""
...
def load_universe_hist() -> dict[pd.Timestamp, list[str]]:
...
def list_available_futures() -> set:
    avail = set()
...
def load_coin_daily(coins) -> dict[str, pd.Series]:
    out = {}
    for c in coins:
        df = load_coin(c + 'USDT')
...
def simulate(events: pd.DataFrame,
             ...
             tx_cost: float = 0.003,
             swap_edge_threshold: int = 1) -> tuple[pd.Series, dict[str, Any]]:
    """Exact M3 lot engine per Codex spec."""
    if n_pick < 1 or n_pick > 10:
        raise ValueError("n_pick out of range")
    need_cols = {'entry_ts','exit_ts','entry_px','exit_px','pnl_pct','bars_held','coin'}
    if not need_cols.issubset(events.columns):
        raise ValueError(f"events missing: {need_cols - set(events.columns)}")
    if not {'equity','cash_ratio','v21_ret','prev_cash'}.issubset(v21_daily.columns):
        raise ValueError("v21_daily missing required columns")
    ...
    stats = {'n_entries':0,'n_natural_exits':0,'n_swaps':0,'n_shrinks':0,
             'n_expands':0,'n_forced_zero':0,'total_tx_cost':0.0,
             'max_cash_overshoot':0.0,'max_slots':0}
    ...
    for date in v21.index:
        ...
        for lot in lots:
            ...
            target = min(slot_cap, max(remaining, 0.0))
            ...
            if lot.entered:
                if target < cur_val - 1e-12:
                    ...
                    stats['total_tx_cost'] += tx
                    stats['n_shrinks'] += 1
                    lot.qty = target / cur_px if target > 0 and cur_px > 0 else 0.0
                    lot.last_px = cur_px
                ...
                qty = target / lot.entry_px if lot.entry_px > 0 else 0.0
                tx = target * tx_cost
...
if __name__ == '__main__':
    ...
    for n in [1, 2, 3]:
        for uni in [10, 15, 20]:
            ...
            print(f'n={n} uni={uni}: {m} entries={stats["n_entries"]} swaps={stats["n_swaps"]} shrinks={stats["n_shrinks"]} expands={stats["n_expands"]}')
