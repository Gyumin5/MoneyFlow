...
GRID_1D = {
    ...
    "snap": [12, 21, 30, 45, 60],  # 3 배수
}
GRID_4H = {
    "sma": [120, 180, 240, 360, 480, 720],
    "ms":  [20, 30, 40, 60, 90, 120],
    "ml":  [120, 240, 480, 720],
    "vol": [("daily", 0.03), ("bar", 0.50)],
    "snap": [21, 30, 60, 84, 90, 168],
}
GRID_2H = {
    "sma": [60, 120, 180, 240, 360, 480],
    "ms":  [10, 20, 30, 40, 60, 90],
    "ml":  [60, 120, 240, 480],
    "vol": [("bar", 0.50), ("daily", 0.03)],
    "snap": [12, 21, 30, 60, 84, 120, 168],
}
...
def preload_all():
    ...
    # patch so run_backtest uses pre-loaded
...
def _run_spot(cfg: dict, anchor: str) -> dict:
    """Spot L=1 single-member backtest via run_current_coin_v20_backtest."""
    import run_current_coin_v20_backtest as spot_bt
...
def _run_futures(cfg: dict, lev: float, anchor: str) -> dict:
    """Futures fixed-leverage single-member via SingleAccountEngine."""
    global _FUT_DATA
    ...
    # m already contains Sharpe/CAGR/MDD/Cal/Liq/Stops/Rebal/_equity
    eq = m.get("_equity")
    if eq is not None:
