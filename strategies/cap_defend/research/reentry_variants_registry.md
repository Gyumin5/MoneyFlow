...
상태: 탐색 종료(2026-07). 22개 변형으로 설계공간이 소진됐고, 추가 발굴은 다중검정 과적합이라 하지 않는다.
아래는 무엇을 이미 재봤는지 확인하는 용도다 — 같은 변형을 다시 돌리지 않기 위해 남긴다.

목적(당시): 최선 1개 수렴이 아니라, F0 baseline + 21개 변형을 전부 백테스트해서 그룹별 tradeoff 표로 결과 비교.
