"""
Cap Defend V24 Recommendation Script (Standard Version)
=======================================================
주식 V24: snap-based 3-tranche stagger (SNAP_PERIOD=69d, STAGGER=23d, N_SNAPS=3)
...
코인 V24: D_SMA42 단일 멤버 (live engine: trade/coin_live_engine.py)
...
- 리포트는 trade_state.json(V24 live state) 스냅샷을 읽어 표시합니다.
"""
...
def get_dynamic_coin_universe(log: list) -> (list, dict):
    print("\n--- 🛰️ Step 1: Coin Universe Selection (V24) ---")
...
def run_stock_strategy_v15(log, all_prices):
    """V24 Stock Strategy: R7 + EEM canary + Z-score3(Sh252) EW + Defense Top3 (executor 단계 가드 없음)"""
    log.append("<h2>📈 주식 포트폴리오 분석 (V24: R7+EEM+Zscore3+Sh252)</h2>")

    # --- Crash Breaker (legacy 신호용 — V24 executor 는 무시) ---
    ...
    if vt is not None and len(vt) >= 10:
        ...
        if stock_crash:
            ...
            if days_ago == 0:
                ...
                log.append(f"<div style='background:#fef7e0;border:2px solid #f9ab00;padding:16px;border-radius:8px;margin:12px 0'>"
                           f"<h3 style='color:#e37400;margin:0'>⏸️ 대기: VT &le; SMA10</h3></div>")
        elif recovered_today:
            log.append(f"<p style='color:#0d904f'><b>✅ Crash 복귀</b></p>")
...
def run_coin_strategy_v20(coin_universe, all_prices, target_date, log):
    """V24 앙상블 표시: trade_state.json의 last_target_snapshot + 멤버 상태를 렌더링."""
    log.append("<h2>🪙 코인 포트폴리오 (V24: D_SMA42 1D 단일 sn=217×7 drift=0.10)</h2>")
    ...
    for m_name in ('D_SMA42',):  # V24: 단일 멤버
        ...
        mrows.append({
            ...
            '마지막 봉': m_st.get('last_bar_ts', 'N/A'),
    ...
    cash_key = 'Cash' if 'Cash' in weights else ('CASH' if 'CASH' in weights else None)
    weights_norm = dict(weights)
    if cash_key and cash_key != CASH_ASSET:
        weights_norm[CASH_ASSET] = weights_norm.pop(cash_key)

    w_rows = [{'자산': t, '비중': f"{w*100:.2f}%"} for t, w in sorted(weights_norm.items(), key=lambda x: -x[1])]
    try:
        log.append("<p><b>[결합 타겟]</b> 멤버 50:50 EW 합산</p>")
        log.append(f"<div class='table-wrap'>{pd.DataFrame(w_rows).to_html(classes='dataframe small-table', index=False)}</div>")
    except Exception:
        pass

    invested = sum(v for k, v in weights_norm.items() if k != CASH_ASSET)
    cash_pct = weights_norm.get(CASH_ASSET, 0.0)
    if cash_pct >= 0.99:
...
if __name__ == "__main__":
    ...
    c_port, c_stat = run_coin_strategy_v20(c_univ, prices, target_date, log)
