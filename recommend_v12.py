"""
Cap Defend V12 Recommendation Script
====================================
V11 = V10 + Health-Check Forced Rebalancing

Key Enhancement:
- If any held coin fails health check (Price <= SMA30 OR 21d Return <= 0),
  trigger immediate rebalancing regardless of turnover threshold.

Generates 'portfolio_result_v11.html'
"""
...
def get_dynamic_coin_universe(log: list) -> (list, dict):
    """
    V12: Get Top 50 coins from LIVE CoinGecko API, filter by Upbit availability.
    Only includes coins that pass Upbit listing, liquidity, and history checks.
    """
    ...
    FETCH_LIMIT = 100  # Fetch 100 to get enough after filtering
    MIN_TRADE_VALUE_KRW = 1_000_000_000  # 10억
    DAYS_TO_CHECK = 260  # 1 year history check
    headers = {"accept": "application/json", "User-Agent": "Mozilla/5.0"}
    
    # --- 1. Fetch from CoinGecko (with retry) ---
    cg_data = []
    max_retries = 3
    for attempt in range(max_retries):
        try:
            print(f"  - Fetching Top {FETCH_LIMIT} from CoinGecko (Attempt {attempt+1}/{max_retries})...")
            ...
            if cg_response.status_code == 200:
                ...
                print(f"    ✅ Got {len(cg_data)} coins from CoinGecko")
                break
            elif cg_response.status_code == 429:
                print(f"    ⚠️ Rate Limit (429). Waiting 10s...")
                time.sleep(10)
            else:
    ...
    # --- 3. Filter: CoinGecko Top -> Stablecoin excluded -> Upbit listed + liquid + history ---
    ...
    for item in cg_data:
        ...
        # Check Upbit liquidity and history
        ...
        try:
            ...
            if ticker_usd in final_universe:
                continue
        ...
        # Stop at Top 50 qualified coins
        if len(final_universe) >= 50:
            break
    
    log.append(f"<p>선정된 유니버스 ({len(final_universe)}개): Live CoinGecko Top 50, filtered by Upbit</p>")
    log.append(f"<p><small>{final_universe[:10]}...</small></p>")
    
    print(f"\n  ✅ Final Universe: {len(final_universe)} qualified coins")
    return final_universe, cg_symbol_to_id_map


# --- 3. Data Download ---
def download_required_data(tickers: list, log: list, coin_id_map: dict):
    print("\n--- 📥 Step 2: 데이터 다운로드 (Yahoo Priority) ---")
    log.append("<h2>📥 Step 2: 데이터 다운로드 (Yahoo 우선)</h2>")
    ...
    for ticker in list(set(tickers)):
        ...
        # 1. Try Yahoo FIRST (User Request for Stability)
        try:
        ...
        # 2. CoinGecko Fallback if Yahoo Failed
        ...
        if not success:
            ...
            log.append(f"<p class='error'>Failed to download: {ticker}</p>")
# --- 4. Logic Engines ---
...
def run_stock_strategy_v11(log, all_prices, target_date):
    """
    V11 Stock: V3 Universe + v1 Canary (200 SMA on VT/EEM)
...
def run_coin_strategy_v12(coin_universe, all_prices, target_date, log, is_today=True):
    """
    V12 Coin: V11 + 10% Vol Cap Filter
    Returns: Portfolio, Status, MetaData (Signal Dist, Next Candidates, Health Status), Log
    """
    date_str = target_date.date()
    log.append(f"<h3>🪙 코인 포트폴리오 (V12: Top 50 + 10% Vol Cap) ({date_str})</h3>")
    ...
    # 2. Health Check + Vol Cap Filter
    healthy = []
    rows = []
    scored_coins = []
    
    def get_volatility(s, n=90):
    ...
    for t in coin_universe:
        ...
        # V12: Vol Cap Filter (10%)
        is_vol_ok = vol90 <= VOL_CAP_FILTER
        is_health_ok = (cur_p > sma30) and (mom21 > 0)
        is_ok = is_health_ok and is_vol_ok
    ...
    for c in scored_coins:
        rows.append({
            ...
            'SMA30': fmt_price(c['SMA30']),
            'Mom21': f"{c['Mom21']:.2%}",
            'Vol90d': f"{c['Vol90d']:.4f}",
            'Score': f"{c['Score']:.2f}",
            'Status': c['Status']
        })
    ...
    # Log Score Table
    log.append("<h4>🏆 최종 선발 (Sharpe V11)</h4>")
...
def save_html(log_global, final_port, s_port, c_port, s_stat, c_stat, turnover, log_today, log_yesterday, date_today, asset_prices_krw, s_meta, c_meta, coin_health_status):
    ...
    # Health Status for V11
    health_json = json.dumps(coin_health_status)
    ...
    <body>
        <div class="container">
            ...
            <p class="small-text">기준일: {date_today.strftime('%Y-%m-%d')} | V11: Health-Check Forced Rebalancing</p>
        ...
        <script>
            ...
            const healthStatus = {health_json};
            ...
            function calcTurnover() {{
               ...
               if(unhealthyCoins.length > 0) {{
                   alertHtml = `<div style="background:#e74c3c; color:white; padding:15px; border-radius:8px; margin:10px 0;">
                       <h3>🚨 즉시 리밸런싱 필요! (V11 Health-Check)</h3>
                       <p>보유 중인 코인이 헬스체크 실패: <strong>${{unhealthyCoins.join(', ')}}</strong></p>
                       <p>턴오버 30% 조건과 관계없이 즉시 매매하세요!</p>
                   </div>`;
               }}
...
if __name__ == "__main__":
    ...
    # Stock Run
    s_port, s_stat, s_meta = run_stock_strategy_v11(log, prices, today)
    
    # Coin Run (Today & Yest)
    log_today = []
    c_port, c_stat, c_meta, log_today = run_coin_strategy_v12(c_univ, prices, today, log_today, True)
    
    # Yesterday for Turnover
    c_port_y, _, _, _ = run_coin_strategy_v12(c_univ, prices, yest, [], False)
    turnover = calculate_turnover(c_port_y, c_port)
    
    # V11: Build coin health status dict for JS
    coin_health_status = {}
    for t in c_univ:
        p = prices.get(t, pd.Series(dtype=float)).loc[:today]
        if len(p) >= 35:
            sma30 = p.rolling(30).mean().iloc[-1]
            mom21 = calc_ret(p, 21)
            # Convert numpy bool_ to Python bool for JSON serialization
            coin_health_status[t] = bool((p.iloc[-1] > sma30) and (mom21 > 0))
        else:
            coin_health_status[t] = False
    ...
    s_port_y, s_stat_y, _ = run_stock_strategy_v11([], prices, yest)
    ...
    save_html(log, final, s_port, c_port, s_stat, c_stat, turnover, log_today, [], today, krw_prices, s_meta, c_meta, coin_health_status)
