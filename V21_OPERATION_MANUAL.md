# V21 운영 매뉴얼 (2026-04-17 확정, 2026-04-21 가드 전면 제거)

## 전환 개요

- 현물 코인: V20 (D_SMA50 + 4h_SMA240 50:50) → V21 (D봉 3멤버 EW 1/3씩), 가드 없음
- 선물: V20 d005 4전략 (동적 lev 3~5x, stop -15%, cash_guard) → V21 고정 3x 3멤버 EW 1/3씩, 가드 없음
- 가드 전면 제거 (2026-04-21): 현물/선물 공히 gap/exclusion/stop/cash_guard 전부 없음. 앙상블 분산 + Upbit warning/delisting 필터(현물)만으로 방어.
- 주식: V17 고정 (변경 없음)
...
- 가드: 없음 (2026-04-21 gap/exclusion 전면 제거). Upbit warning/delisting 코인은 universe 단계에서 이미 제외됨.
...
다른 키는 유지 (last_combined, last_target_snapshot 등).
`excluded_coins` 키는 가드 제거 이후 엔진이 매 실행마다 자동 삭제하므로 state에서 사라진다.
...
7. V21 초기 state 생성 (canary OFF, excluded_coins 없음)
8. scp로 V21 파일 배포
9. 서버 cron 교체:
   - 코인: `5 9 * * *`
   - 선물: `5 9,13,17,21,1,5 * * *`
...
| 현물 가드 | gap -15% / excl 30d | 없음 (2026-04-21 제거) |
